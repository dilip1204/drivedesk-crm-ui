...
const Instructors = () => {
  ...
  const getInstructorsList = () => {
    const data = {};
    dispatch(getInstructorsListInformation(data, (res) => {
  ...
  const deleteData = (appId) => {
    ...
    dispatch(
      deleteStudent(payloadDeleteInstructor, (res) => {
  ...
  const onInstructorsData = (res, isEdit) => {
    if(!res.isError) {
          toast.success(isEdit ? 'Instructor updated successfully!':'Instructor added successfully!');
        }else {
          toast.error('Failed....!')
    }
  }
                  ...
                  <div className="breadcrumb-wrapper col-xl-6">
                    ...
                    <nav aria-label="breadcrumb">
                      <ol className="breadcrumb p-0">
                        ...
                        <li className="breadcrumb-item" aria-current="page">InstructorsList</li>
                      </ol>
                    </nav>
                  </div>
      ...
      {instructorsData.map((ins, index) => (
        <div className="col-xl-3 col-lg-4 col-md-6 col-sm-12 mb-3" key={index}>
          <div className="student-card position-relative">
            ...
            >
              <button className="btn btn-sm btn-warning" title="Edit Instructor" onClick={() => handleEditInstructor(ins)}>
                <i className="bi bi-pencil"></i>
              </button>
              <button className="btn btn-sm btn-danger" title="Delete INstructor" onClick={()=>deleteUser(ins.mobile_number)}>
            ...
            <div>
              ...
              <div className="completed-classes">
                <i className="bi bi-check-circle"></i> {ins.available_from } to {ins.available_to}
              </div>
            </div>
          </div>
        </div>
